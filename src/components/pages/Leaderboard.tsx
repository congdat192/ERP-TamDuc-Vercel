...
export function Leaderboard() {
  ...
  const getBadgeIcon = (badge: string | null) => {
    ...
    if (badge.includes('Champion')) return <Trophy className="w-4 h-4" />;
    if (badge.includes('Star')) return <Star className="w-4 h-4" />;
    if (badge.includes('Conversion')) return <Target className="w-4 h-4" />;
    ...
    <div className="space-y-6">
      ...
      <div className="text-center space-y-2">
        <h2 className="text-3xl font-bold text-gray-900 flex items-center justify-center space-x-3">
          ...
          <span>Telesales Leaderboard</span>
        </h2>
        <p className="text-gray-600">Track performance and celebrate achievements</p>
      </div>
                ...
                {getCurrentLeaderboard().slice(0, 3).map((person, index) => {
                  ...
                  return (
                    <div key={person.name} className={`flex flex-col items-center ${index === 1 ? 'order-1' : index === 0 ? 'order-2' : 'order-3'}`}>
                      ...
                      <Badge variant="secondary" className="mb-2">{person.issued} vouchers</Badge>
                      <div className={`${heights[index]} w-20 bg-gradient-to-t ${
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-center">
                  ...
                  <h4 className="font-semibold text-yellow-800">Daily Champion</h4>
                  <p className="text-sm text-yellow-600">Sarah Johnson - 23 vouchers issued</p>
                </div>
                <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
                  <Target className="w-8 h-8 text-green-500 mx-auto mb-2" />
                  <h4 className="font-semibold text-green-800">Best Conversion</h4>
                  <p className="text-sm text-green-600">Sarah Johnson - 91% conversion rate</p>
                </div>
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-center">
                  <Star className="w-8 h-8 text-blue-500 mx-auto mb-2" />
                  <h4 className="font-semibold text-blue-800">Rising Star</h4>
                  <p className="text-sm text-blue-600">Tom Wilson - +3 rank improvement</p>
                </div>
              </div>
            </CardContent>
          </Card>
