...
export function MultiSelectFilter({ 
  ...
  return (
    <div className="space-y-2">
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            role="combobox"
            aria-expanded={open}
            className="voucher-input h-9 text-sm rounded-md w-full justify-between theme-border-primary"
          >
      ...
      {/* Selected tags - only show if there are selections */}
      {selectedValues.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {selectedValues.slice(0, 3).map((value) => {
            ...
            return (
              <Badge
                key={value}
                variant="secondary"
                className="text-xs h-5 px-2 theme-bg-primary/10 theme-text-primary rounded-md"
              >
                {option?.label || value}
                <X
          ...
          {selectedValues.length > 3 && (
            <Badge variant="secondary" className="text-xs h-5 px-2 theme-bg-primary/10 theme-text-primary rounded-md">
