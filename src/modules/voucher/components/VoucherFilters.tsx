...
export function VoucherFilters({ filters, onFiltersChange, availableIssuers }: VoucherFiltersProps) {
  ...
  const toggleMultiSelectValue = (key: 'status' | 'issuedBy' | 'denomination', value: string) => {
        ...
        <Popover open={isFiltersOpen} onOpenChange={setIsFiltersOpen}>
          ...
          <PopoverContent className="w-96 p-6" align="end">
            <div className="space-y-6">
                    ...
                    <Badge
                      ...
                      className={`cursor-pointer ${
                        filters.status.includes(option.value) 
                          ? 'voucher-bg-primary voucher-text-white' 
                          : 'hover:voucher-bg-primary/10'
                      }`}
                      onClick={() => toggleMultiSelectValue('status', option.value)}
                    >
                      {option.label}
                    </Badge>
                  ))}
              ...
              {/* Denomination Filter */}
              <div>
                <Label className="text-sm font-medium theme-text-muted">Mệnh Giá</Label>
                <div className="mt-2 flex flex-wrap gap-2">
                  {denominationOptions.map((option) => (
                    <Badge
                      key={option.value}
                      variant={filters.denomination.includes(option.value) ? "default" : "outline"}
                      className={`cursor-pointer ${
                        filters.denomination.includes(option.value) 
                          ? 'voucher-bg-primary voucher-text-white' 
                          : 'hover:voucher-bg-primary/10'
                      }`}
                      onClick={() => toggleMultiSelectValue('denomination', option.value)}
                    >
                      {option.label}
                    </Badge>
                  ))}
              ...
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {availableIssuers.map((issuer) => (
                    <Badge
                      key={issuer}
                      variant={filters.issuedBy.includes(issuer) ? "default" : "outline"}
                      className={`cursor-pointer ${
                        filters.issuedBy.includes(issuer) 
                          ? 'voucher-bg-primary voucher-text-white' 
                          : 'hover:voucher-bg-primary/10'
                      }`}
                      onClick={() => toggleMultiSelectValue('issuedBy', issuer)}
                    >
                      {issuer}
                    </Badge>
                  ))}
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div>
                  <Label className="text-sm font-medium theme-text-muted">KH Phát Sinh Hóa Đơn</Label>
              ...
              {/* Value Range Filter */}
              <div>
                <Label className="text-sm font-medium theme-text-muted">Khoảng Giá Trị</Label>
                <div className="mt-2 flex gap-2">
                  <Input
                    placeholder="Từ (VNĐ)"
          ...
          {filters.status.map((status) => (
            <Badge key={status} variant="secondary" className="flex items-center gap-1">
              ...
              <X 
                ...
                onClick={() => toggleMultiSelectValue('status', status)}
              />
            </Badge>
          ))}
          {filters.denomination.map((denom) => (
            <Badge key={denom} variant="secondary" className="flex items-center gap-1">
              {denominationOptions.find(opt => opt.value === denom)?.label}
              <X 
                className="w-3 h-3 cursor-pointer" 
                onClick={() => toggleMultiSelectValue('denomination', denom)}
              />
            </Badge>
          ))}
