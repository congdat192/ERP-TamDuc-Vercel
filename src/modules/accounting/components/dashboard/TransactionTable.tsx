        ...
        <Card>
            <CardHeader className="pb-3">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <CardTitle className="flex items-center gap-2 text-lg"><History size={20} className="text-gray-400" /> Giao dịch</CardTitle>
                    <Button
                        variant={isFilterOpen || isFilterActive ? "secondary" : "outline"}
                        size="sm"
            ...
            {isFilterOpen && (
                <div className="px-6 pb-4 bg-gray-50/50 border-b border-gray-100 animate-in slide-in-from-top-2 duration-200">
                    <div className="flex flex-wrap items-center gap-2 mb-4 pt-4">
                        ...
                        <Button variant="outline" size="sm" onClick={() => applyDatePreset('TODAY')} className="h-7 text-xs">Hôm nay</Button>
                        <Button variant="outline" size="sm" onClick={() => applyDatePreset('THIS_WEEK')} className="h-7 text-xs">Tuần này</Button>
                        <Button variant="outline" size="sm" onClick={() => applyDatePreset('THIS_MONTH')} className="h-7 text-xs">Tháng này</Button>
                        <div className="flex-1"></div>
                        {isFilterActive && <Button variant="ghost" size="sm" onClick={resetFilters} className="h-7 text-xs text-rose-600 hover:text-rose-700 hover:bg-rose-50"><X size={14} className="mr-1" /> Xóa bộ lọc</Button>}
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                        <div><label className="block text-xs font-semibold text-gray-500 mb-1">Từ ngày</label><Input type="date" value={filterStartDate} onChange={(e) => setFilterStartDate(e.target.value)} className="h-9 text-sm" /></div>
                        <div><label className="block text-xs font-semibold text-gray-500 mb-1">Đến ngày</label><Input type="date" value={filterEndDate} onChange={(e) => setFilterEndDate(e.target.value)} className="h-9 text-sm" /></div>
                        <div>
                            <label className="block text-xs font-semibold text-gray-500 mb-1">Loại giao dịch</label>
                            <select
                                value={filterType}
                                onChange={(e) => { setFilterType(e.target.value as TransactionType | 'ALL'); setFilterCategory('ALL'); }}
                                className="w-full h-9 px-3 py-1 text-sm bg-white border border-gray-200 rounded-md focus:ring-2 focus:ring-indigo-500 outline-none"
                            >
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-gray-500 mb-1">Loại quỹ</label>
                            <select
                                value={filterFundType}
                                onChange={(e) => setFilterFundType(e.target.value as FundType | 'ALL')}
                                className="w-full h-9 px-3 py-1 text-sm bg-white border border-gray-200 rounded-md focus:ring-2 focus:ring-indigo-500 outline-none"
                            >
                                <option value="ALL">Tất cả</option>
                                <option value={FundType.CASH}>Tiền mặt</option>
                                <option value={FundType.BANK}>Ngân hàng</option>
                            </select>
                        </div>
                        <div>
                            ...
                            >
                                ...
                                {filterType === 'ALL' ? (
                                    <>
                                        <optgroup label="Thu">{categories[TransactionType.INCOME].map(cat => <option key={cat} value={cat}>{cat}</option>)}</optgroup>
                                        <optgroup label="Chi">{categories[TransactionType.EXPENSE].map(cat => <option key={cat} value={cat}>{cat}</option>)}</optgroup>
                                    </>
                        ...
                        {tableTransactions.length > 0 ? tableTransactions.slice(0, isFilterActive ? 50 : 10).map((tx) => (
                            <tr key={tx.id} className="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td className="px-6 py-4 text-center">
                                    <Button variant="ghost" size="icon" onClick={() => onDelete(tx.id)} title="Xóa" className="text-gray-400 hover:text-rose-500 hover:bg-rose-50 h-8 w-8"><Trash2 size={16} /></Button>
                                </td>
                            </tr>
