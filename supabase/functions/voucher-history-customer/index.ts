...
async function getOAuthToken(): Promise<string> {
  console.log('[voucher-history-customer] Fetching OAuth token...');
  ...
  console.log('[voucher-history-customer] OAuth token fetched successfully');
...
serve(async (req) => {
  ...
  try {
    ...
    if (!externalApiKey || !clientId || !clientSecret) {
      console.error('[voucher-history-customer] Missing required secrets');
      return new Response(
        JSON.stringify({ 
    ...
    console.log('[voucher-history-customer] Fetching history for phone:', phone);
    ...
    const response = await fetch(
      `${EXTERNAL_API_BASE}/voucher-history-customer?${params.toString()}`,
      {
        method: 'GET',
        headers: {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('[voucher-history-customer] API error:', response.status, errorText);
      return new Response(
        JSON.stringify({ 
    ...
    const data = await response.json();
    console.log('[voucher-history-customer] Success:', data.data?.pagination);
    ...
    console.error('[voucher-history-customer] Exception:', error);
    return new Response(
      JSON.stringify({ 
